<template>
    <div class="modal fade" id="mesaPersoneroModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        data-backdrop="static" data-keyboard="false" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-simple modal-enable-otp">
            <div class="modal-content p-3">
                <div class="modal-header">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    <h5 class="modal-title" v-if="option"> MESA - PERSONERO</h5>
                    <h5 class="modal-title" v-else>Agregar MESA | {{ centro.nombre }}</h5>
                </div>
                                        ...
                                        <div class="col-md-12 table-responsive text-nowrap">
                                            <table class="table table-hover table-sm">
                                                ...
                                                <tbody class="table-border-bottom-0">
                                                    <tr v-for="(item, index) in personeros.data" :key="item.id">
...
export default {
    ...
    methods: {
        ...
        async fetchUserList(page = 1) {
            try {
                const result = await Services.getListInfo(this.searchQuery, `mesa/list-personeros-mesa?page=${page}&mesa_id=${this.mesa.id}`, this.pageSize);
