...
class PersonaController extends Controller
{
    ...
    public function getListUsers(Request $request)
    {
        ...
        $users = User::rightJoin('personas as p', 'users.persona_id', '=', 'p.id')
            ->leftJoin('perfiles as p2', 'users.perfil_id', '=', 'p2.id')
            ->where(function ($query) use ($search) {
                $query->where('p.apellido_paterno', 'LIKE', '%' . $search . '%')
                    ->orWhere('p.apellido_materno', 'LIKE', '%' . $search . '%')
                    ->orWhere('p.email', 'LIKE', '%' . $search . '%')
                    ->orWhere('p2.nombre', 'LIKE', '%' . $search . '%');
            })
            ->select(
                'p.id',
                'p.nombre',
        ...
        /* $users = Person::join('users', 'users.persona_id', '=', 'personas.id')
            ->join()
            ->select(
                'personas.id',
                'personas.nombre',
